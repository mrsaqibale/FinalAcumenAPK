...
import 'package:acumen/features/business/screens/mentor_quiz_results_screen.dart';
import 'package:acumen/theme/app_theme.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

class MentorQuizTabsScreen extends StatefulWidget {
  const MentorQuizTabsScreen({Key? key}) : super(key: key);

  @override
  State<MentorQuizTabsScreen> createState() => _MentorQuizTabsScreenState();
}

class _MentorQuizTabsScreenState extends State<MentorQuizTabsScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Load real quiz data when screen initializes
  ...
  Future<void> _refreshQuizzes() async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('Quizzes refreshed: ${controller.quizzes.length} quizzes found');
      }
    } catch (e) {
      if (kDebugMode) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error refreshing quizzes: $e')),
        );
      }
    } finally {
      if (mounted) {
    ...
    return Consumer<QuizController>(
      builder: (context, quizController, _) {
        return Column(
          ...
          children: [
                        // MY QUIZZES Tab
                        _buildMyQuizzesTab(quizController),
                        
                        // RESULTS Tab
            const MentorQuizResultsScreen(),
          ],
        ),
      ),
            ),
  ...
  Widget _buildMyQuizzesTab(QuizController quizController) {
                        ...
                        PopupMenuButton<String>(
                          ...
                          onSelected: (value) {
                            if (value == 'edit') {
                              ...
                              _showDeleteConfirmation(context, quizController, quiz['id']);
  ...
  void _showDeleteConfirmation(BuildContext context, QuizController controller, String quizId) {
    showDialog(
      context: context,
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(success ? 'Quiz deleted successfully' : 'Error deleting quiz')),
              );
              }
            },
            style: ElevatedButton.styleFrom(
